...
__all__ = [
    "CeramicStatisticalAnalyzer",
    "CeramicVisualization",
    "quick_ceramic_analysis",
    "compare_assemblages",
]
...
class CeramicStatisticalAnalyzer:
    ...
    def perform_pca(
        self, n_components: int | None = None, variance_threshold: float = 0.95
    ) -> dict:
        """
        ...
        loadings_df = pd.DataFrame(loadings, columns=pc_names, index=self.feature_names)
    ...
    def _interpret_pca_components(self, loadings_df: pd.DataFrame) -> dict:
        ...
        for pc in loadings_df.columns:
            ...
            size_features = [
                ...
                if any(x in f.lower() for x in ["size", "area", "diameter", "length"])
            ]
            color_features = [
                f
                for f in top_features.index
                if any(x in f.lower() for x in ["color", "rgb", "lab", "hue"])
            ]
            orientation_features = [
                f
                for f in top_features.index
                if any(x in f.lower() for x in ["orientation", "angle", "alignment"])
            ]
            geometric_features = [
                f
                for f in top_features.index
                if any(x in f.lower() for x in ["geometric", "roundness", "angularity"])
            ]
    ...
    def _get_archaeological_meaning(self, component: str, top_features: pd.Series) -> str:
        ...
        if any("size" in f for f in feature_names):
            ...
            return (
                "Fabric structure axis: relates to manufacturing technique and clay working methods"
            )
    ...
    def correlation_analysis(
                ...
                if i != j:
                    if method == "pearson":
                        _, p_val = stats.pearsonr(
                            self.scaled_data[feature1], self.scaled_data[feature2]
                        )
                    elif method == "spearman":
        ...
        p_values_df = pd.DataFrame(p_values, index=self.feature_names, columns=self.feature_names)
    ...
    def assemblage_comparison(self, group_column: str, comparison_type: str = "all") -> dict:
        """
        ...
        raise NotImplementedError(
            "Assemblage comparison requires group data - implement based on specific needs"
        )
    ...
    def _generate_archaeological_summary(self, report: dict) -> dict:
        ...
        summary = {"main_findings": [], "technological_insights": [], "recommendations": []}
...
def quick_ceramic_analysis(data: pd.DataFrame | dict, scaling_method: str = "standard") -> dict:
    """
...
class CeramicVisualization:
    ...
    def plot_pca_biplot(
        ...
        fig.update_layout(showlegend=False, width=800, height=600, template="plotly_white")
    ...
    def plot_pca_3d(
        ...
        if cluster_labels is not None:
            ...
            scores_df["Cluster"] = [
                f"Cluster {label + 1}" if label >= 0 else "Noise" for label in cluster_labels
            ]
    ...
    def plot_scree_plot(pca_results: dict) -> go.Figure:
        ...
        fig.add_hline(y=95, line_dash="dash", line_color="gray", annotation_text="95% Variance")
    ...
    def plot_dendrogram(
        ...
        scipy_color_keys = sorted(
            ...
            key=lambda c: dend["color_list"].index(c) if c in dend["color_list"] else 999,
        )
        color_map = {key: palette[i % len(palette)] for i, key in enumerate(scipy_color_keys)}
        ...
        for i, d, sc in zip(dend["icoord"], dend["dcoord"], dend["color_list"]):
            fig.add_trace(
                go.Scatter(
                    x=i,
                    y=d,
    ...
    def plot_correlation_heatmap(correlation_results: dict) -> go.Figure:
        ...
        fig.update_layout(
            title="Feature Correlation Matrix", width=800, height=800, template="plotly_white"
        )
    ...
    def plot_cluster_comparison(
        cluster_results: dict, feature_data: pd.DataFrame, features_to_plot: list[str] = None
    ) -> go.Figure:
        """
        ...
        fig = make_subplots(
            rows=rows, cols=cols, subplot_titles=features_to_plot, vertical_spacing=0.1
        )
        ...
        fig.update_layout(
            title="Cluster Feature Comparison", height=300 * rows, template="plotly_white"
        )
    ...
    def plot_archaeological_summary(report: dict) -> go.Figure:
        ...
        if "cluster_analysis" in report and "silhouette_score" in report["cluster_analysis"]:
            ...
            fig.add_trace(
                go.Bar(x=metrics, y=values, name="Quality", marker_color="lightgreen"), row=1, col=2
            )

        # Top correlations (if available)
        if (
            "correlation_analysis" in report
            and "significant_correlations" in report["correlation_analysis"]
        ):
        ...
        if "data_summary" in report:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(
                        size=[n_samples / 5, n_features / 2, 20], color=["blue", "red", "green"]
                    ),
                    name="Data Overview",
                ),
                row=2,
...
def compare_assemblages(
    data: pd.DataFrame, group_column: str, analysis_type: str = "comprehensive"
) -> dict:
    """
    ...
    raise NotImplementedError(
        "Assemblage comparison functionality - implement based on specific research needs"
    )
